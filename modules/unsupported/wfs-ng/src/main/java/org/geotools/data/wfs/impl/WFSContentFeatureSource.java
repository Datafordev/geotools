...
public class WFSContentFeatureSource extends ContentFeatureSource {
    ...
    public WFSContentFeatureSource(final ContentEntry entry, final WFSStrategy wfsImpl) {
        super(entry, null);
    ...
    protected ReferencedEnvelope getBoundsInternal(Query query) throws IOException {
        ...
        if (null == query.getCoordinateSystem()) {
            targetCrs = getFeatureTypeCRS(typeName);
        } else {
        ...
        try {
            nativeBounds = wgs84Bounds.transform(targetCrs, true);
        } catch (TransformException e) {
            LOGGER.log(
                    Level.WARNING,
                    "Can't transform bounds of " + typeName + " to "
                            + wfsImpl.getDefaultCRS(typeName), e);
            nativeBounds = new ReferencedEnvelope(targetCrs);
    ...
    protected FeatureReader<SimpleFeatureType, SimpleFeature> getReaderInternal(Query localQuery)
        ...
        Integer maxFeaturesHardLimit = wfsImpl.getConfig().getMaxFeatures();
    ...
    protected int getMaxFeatures(Query query) {
        ...
        int maxFeaturesDataStoreLimit = maxFeaturesHardLimit == null ? Query.DEFAULT_MAX
    ...
    private boolean useHttpPost() {
        boolean preferPostOverGet = wfsImpl.getConfig().isPreferPostOverGet();
