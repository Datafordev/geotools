...
/**
 * Testing the ability of WFS to turn transaction state into a series of actions for encoding into a
 * Transaction request.
 * <p>
 * This test focuses on ability to combine a bunch of update actions for the same feature into a
 * single action.
 * 
 * @author Jesse
 * 
...
public class ProcessEditsBeforeCommitTest extends TestCase {
    ...
    static {
        CUSTOM_FILTERALL = filterFac.equals(filterFac.createLiteralExpression(1),
                filterFac.createLiteralExpression(2));
    }
    ...
    protected void setUp() throws Exception {
        ...
        typename = "Type";
        featureType = DataUtilities.createType(typename, GEOM_ATT + ":Point," + NAME_ATT
                + ":String");
    }
    ...
    public void testBasicInsertFIDFilterUpdateCollapse() throws Exception {
        ...
        state.addAction(featureType.getTypeName(),
                new Action.UpdateAction(typename, filterFac.createFidFilter(createFeature.getID()),
                        properties));

        state.combineActions();
        Action.InsertAction action = (InsertAction) state.getActions(featureType.getTypeName())
                .get(0);
        assertEquals(1, state.getActions(featureType.getTypeName()).size());
        assertEquals("newName", action.getFeature().getAttribute(NAME_ATT));
        assertTrue(((Point) createFeature.getAttribute(GEOM_ATT)).equals((Point) action
                .getFeature().getAttribute(GEOM_ATT)));
    }
    ...
    public void testMultiInsertUpdateCollapse() throws Exception {
        ...
        state.addAction(featureType.getTypeName(), new Action.InsertAction(createFeature));
        state.addAction(featureType.getTypeName(),
                new Action.InsertAction(createFeature(10, 10, NAME_ATT)));
        Map properties = new HashMap();
        properties.put(NAME_ATT, "newName");
        state.addAction(featureType.getTypeName(),
                new Action.UpdateAction(typename, filterFac.createFidFilter(createFeature.getID()),
    ...
    public void testFidAndNonFidFilterUpdates() throws Exception {
        ...
        state.addAction(featureType.getTypeName(), new Action.UpdateAction(typename,
                Filter.INCLUDE, updateProperties));
        state.addAction(featureType.getTypeName(), new Action.InsertAction(createFeature));
        ...
        state.addAction(featureType.getTypeName(), new Action.UpdateAction(typename, matchFilter,
                updateProperties));

        updateProperties.put(NAME_ATT, "secondNoMatch");
        state.addAction(featureType.getTypeName(), new Action.UpdateAction(typename,
                CUSTOM_FILTERALL, updateProperties));
    ...
    public void testInsertAndDelete() throws Exception {
        ...
        state.addAction(featureType.getTypeName(), new Action.InsertAction(createFeature));
        state.addAction(featureType.getTypeName(),
                new Action.DeleteAction(typename, filterFac.createFidFilter(createFeature.getID())));
    ...
    public void testUpdatesDeletesAndInsertes() throws Exception {
        ...
        Action.UpdateAction updateAll = new Action.UpdateAction(typename, Filter.INCLUDE,
                updateProperties);

        updateProperties.put(NAME_ATT, "newName");
        Action.UpdateAction updateInsert1 = new Action.UpdateAction(typename,
                filterFac.createFidFilter(createFeature.getID()), updateProperties);

        updateProperties.put(NAME_ATT, "secondUpdate");
        CompareFilter matchFilter = filterFac.createCompareFilter(FilterType.COMPARE_EQUALS);
        matchFilter.addLeftValue(filterFac.createAttributeExpression("name"));
        matchFilter.addRightValue(filterFac.createLiteralExpression("newName"));
        Action.UpdateAction updateManyIncludingInsert1 = new Action.UpdateAction(typename,
                matchFilter, updateProperties);

        updateProperties.put(NAME_ATT, "secondNoMatch");
        Action.UpdateAction updateNone = new Action.UpdateAction(typename, CUSTOM_FILTERALL,
                updateProperties);
